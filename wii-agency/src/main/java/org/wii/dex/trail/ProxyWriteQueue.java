...
public class ProxyWriteQueue {
    ...
    private final Queue<ProxyWriteQueue.QueuedCommand> queue;
    private final AtomicBoolean scheduled = new AtomicBoolean();

    public ProxyWriteQueue(Channel channel) {
        this.channel = Preconditions.checkNotNull(channel, "channel");
        queue = new ConcurrentLinkedQueue<ProxyWriteQueue.QueuedCommand>();
    }
    ...
    ChannelFuture enqueue(ProxyWriteQueue.QueuedCommand command, boolean flush) {
     ...
     */
    @CanIgnoreReturnValue
    ChannelFuture enqueue(ProxyWriteQueue.QueuedCommand command, ChannelPromise promise, boolean flush) {
    ...
    void enqueue(Runnable runnable, boolean flush) {
        queue.add(new ProxyWriteQueue.RunnableCommand(runnable));
        if (flush) {
            scheduleFlush();
        }
    }
    ...
    private void flush() {
        try {
            ProxyWriteQueue.QueuedCommand cmd;
            int i = 0;
    ...
    private static class RunnableCommand implements ProxyWriteQueue.QueuedCommand {
    ...
    abstract static class AbstractQueuedCommand implements ProxyWriteQueue.QueuedCommand {
