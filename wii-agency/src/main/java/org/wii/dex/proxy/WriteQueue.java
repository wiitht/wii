...
public class WriteQueue {
    ...
    private final Queue<WriteQueue.QueuedCommand> queue;
    private final AtomicBoolean scheduled = new AtomicBoolean();

    public WriteQueue(Channel channel) {
        this.channel = Preconditions.checkNotNull(channel, "channel");
        queue = new ConcurrentLinkedQueue<WriteQueue.QueuedCommand>();
    }
    ...
    public ChannelFuture enqueue(WriteQueue.QueuedCommand command, boolean flush) {
     ...
     */
    @CanIgnoreReturnValue
    ChannelFuture enqueue(WriteQueue.QueuedCommand command, ChannelPromise promise, boolean flush) {
    ...
    void enqueue(Runnable runnable, boolean flush) {
        queue.add(new WriteQueue.RunnableCommand(runnable));
        if (flush) {
            scheduleFlush();
        }
    }
    ...
    private static class RunnableCommand implements WriteQueue.QueuedCommand {
    ...
    abstract static class AbstractQueuedCommand implements WriteQueue.QueuedCommand {
