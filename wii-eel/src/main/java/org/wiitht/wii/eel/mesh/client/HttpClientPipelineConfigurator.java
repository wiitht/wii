...
/**
 * @Author tanghong
 * @Date 18-10-23-下午5:09
 * @Version 1.0
 */
public class HttpClientPipelineConfigurator extends ChannelDuplexHandler {
    ...
    private final HttpClientPipelineConfigurator.HttpPreference httpPreference;
    @Nullable
    ...
    HttpClientPipelineConfigurator(HttpClientFactory clientFactory, SessionProtocol sessionProtocol) {
        ...
        if (sessionProtocol == HTTP || sessionProtocol == HTTPS) {
            httpPreference = HttpClientPipelineConfigurator.HttpPreference.HTTP2_PREFERRED;
        } else if (sessionProtocol == H1 || sessionProtocol == H1C) {
            httpPreference = HttpClientPipelineConfigurator.HttpPreference.HTTP1_REQUIRED;
        } else if (sessionProtocol == H2 || sessionProtocol == H2C) {
            httpPreference = HttpClientPipelineConfigurator.HttpPreference.HTTP2_REQUIRED;
        } else {
        ...
        if (sessionProtocol.isTls()) {
            try {
                ...
                if (httpPreference == HttpClientPipelineConfigurator.HttpPreference.HTTP2_REQUIRED ||
                        httpPreference == HttpClientPipelineConfigurator.HttpPreference.HTTP2_PREFERRED) {
        ...
        p.addLast(new ChannelInboundHandlerAdapter() {
            ...
            public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
                ...
                if (isHttp2Protocol(sslHandler)) {
                    if (httpPreference == HttpClientPipelineConfigurator.HttpPreference.HTTP1_REQUIRED) {
    ...
    private void configureAsHttp(Channel ch) {
        ...
        if (attemptUpgrade) {
            ...
            if (clientFactory.useHttp2Preface()) {
                pipeline.addLast(new HttpClientPipelineConfigurator.DowngradeHandler());
                ...
                pipeline.addLast(new HttpClientPipelineConfigurator.WorkaroundHandler());
                pipeline.addLast(http2UpgradeHandler);
                pipeline.addLast(new HttpClientPipelineConfigurator.UpgradeRequestHandler(http2Handler.responseDecoder()));
            }
        } else {
    ...
    private final class UpgradeRequestHandler extends ChannelInboundHandlerAdapter {
        ...
        public void channelActive(ChannelHandlerContext ctx) throws Exception {
            ...
            res.subscribe(new Subscriber<com.linecorp.armeria.common.HttpObject>() {
                ...
                @Override
                public void onNext(com.linecorp.armeria.common.HttpObject o) {
                    if (notified) {
                    ...
                    notified = true;
                    assert upgradeEvt == HttpClientUpgradeHandler.UpgradeEvent.UPGRADE_SUCCESSFUL;
        ...
        public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
            ...
            final HttpClientUpgradeHandler.UpgradeEvent upgradeEvt = (HttpClientUpgradeHandler.UpgradeEvent) evt;
        ...
        private void onUpgradeResponse(ChannelHandlerContext ctx, boolean success) {
            final HttpClientUpgradeHandler.UpgradeEvent upgradeEvt = this.upgradeEvt;
