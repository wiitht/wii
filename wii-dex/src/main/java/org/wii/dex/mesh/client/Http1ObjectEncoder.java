...
/**
 * @Author tanghong
 * @Date 18-10-24-下午6:49
 * @Version 1.0
 */
public class Http1ObjectEncoder extends HttpObjectEncoder {
    ...
    private HttpObject convertClientHeaders(int streamId, HttpHeaders headers, boolean endStream)
        ...
        final com.linecorp.armeria.common.HttpMethod method = headers.method();
        if (method == null) {
            return convertTrailingHeaders(streamId, headers);
        }
    ...
    private void convert(
        ...
        outHeaders.remove(HttpConversionUtil.ExtensionHeaderNames.STREAM_ID.text());
        if (server) {
            outHeaders.remove(HttpConversionUtil.ExtensionHeaderNames.SCHEME.text());
        } else {
            outHeaders.remove(HttpConversionUtil.ExtensionHeaderNames.PATH.text());
        }
    }
    ...
    private ChannelFuture write(int id, HttpObject obj, boolean endStream) {
        ...
        if (id == currentId) {
            ...
            final Map.Entry<HttpObject, ChannelPromise> entry = new AbstractMap.SimpleImmutableEntry<>(obj, promise);
    ...
    private void flushPendingWrites(PendingWrites pendingWrites) {
        for (;;) {
            final Map.Entry<HttpObject, ChannelPromise> e = pendingWrites.poll();
            if (e == null) {
                break;
            }
    ...
    protected void doClose() {
        ...
        for (Queue<Map.Entry<HttpObject, ChannelPromise>> queue : pendingWritesMap.values()) {
            for (;;) {
                final Map.Entry<HttpObject, ChannelPromise> e = queue.poll();
                if (e == null) {
                    break;
                }
    ...
    private static final class PendingWrites extends ArrayDeque<Map.Entry<HttpObject, ChannelPromise>> {
