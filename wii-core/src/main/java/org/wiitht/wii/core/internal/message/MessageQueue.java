...
public class MessageQueue {
    ...
    public ChannelFuture enqueue(MessageQueue.QueuedCommand command, boolean flush) {
     ...
     */
    @CanIgnoreReturnValue
    public ChannelFuture enqueue(MessageQueue.QueuedCommand command, ChannelPromise promise, boolean flush) {
    ...
    public void enqueue(Runnable runnable, boolean flush) {
        queue.add(new MessageQueue.RunnableCommand(runnable));
        if (flush) {
            scheduleFlush();
        }
    }
    ...
    private void flush() {
        try {
            MessageQueue.QueuedCommand cmd;
            int i = 0;
    ...
    private static class RunnableCommand implements MessageQueue.QueuedCommand {
    ...
    public abstract static class AbstractQueuedCommand implements MessageQueue.QueuedCommand {
